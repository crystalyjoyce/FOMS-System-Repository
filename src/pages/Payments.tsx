...
export const Payments: React.FC = () => {
  ...
  if (isAssistant) allowedPayments = payments.filter(p => p.status === 'Pending Validation');
  else if (isFinanceManager) allowedPayments = payments.filter(p => p.status === 'Validated');

  const filteredPayments = selectedClientId ? allowedPayments.filter(p => p.clientId === selectedClientId) : allowedPayments;
  ...
  const filteredSpeedPay = selectedClientId ? speedPay.filter(s => {
  ...
  let enriched: any[] = [];
  if (selectedClientId) {
    enriched = allowedPayments.filter(p => p.clientId === selectedClientId).map(p => {
  ...
  const fmActions = [
    ...
    },
    {
      label: 'View Receipt',
      icon: 'ti-receipt',
      onClick: (row: any) => setViewReceiptPayment(row),
      hidden: (row: any) => row.status !== 'Approved',
    },
    {
      label: 'View Details',
      icon: 'ti-eye',
      onClick: (row: any) => toast.info(`Remarks: ${row.rejectionReason || 'No reason provided.'}`, 'Rejection Details'),
  ...
  const assistantActions = [
    {
      ...
      onClick: (row: any) => {
        setViewPayment(row);
        setVerificationStatus('Validate');
        setRejectionReason('');
      },
    ...
    },
    {
      label: 'View Details',
      icon: 'ti-eye',
      onClick: (row: any) => toast.info(`Rejection Reason: ${row.rejectionReason || 'No reason provided.'}`, 'Rejection Details'),
      hidden: (row: any) => row.status !== 'Rejected',
    }
  ];
  ...
  else if (isAssistant) actions = assistantActions;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {isFinanceManager ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          <StatusCard label="Total Verified Payments" value={`₱${totalVerified.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`} icon="ti-shield-check" variant="success" />
          <StatusCard label="Pending Final Approval" value={pendingFinal} icon="ti-clock-hour-4" variant="warning" />
          <StatusCard label="Priority Methods Split" value={`${checkCount} Checks / ${obtCount} OBT`} icon="ti-chart-pie" variant="info" />
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
          <StatusCard label="Total Verified Payments" value={`₱${totalVerified.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`} icon="ti-shield-check" variant="success" />
          <StatusCard label="Pending SpeedPay Validations" value={pendingSpeedPay} icon="ti-clock-hour-4" variant="warning" />
          <StatusCard label="Priority Methods Split" value={`${checkCount} Checks / ${obtCount} OBT`} icon="ti-chart-pie" variant="info" />
        </div>
      )}
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <select required value={form.invoiceId} onChange={e => {
                  ...
                  {unpaidInvoices.map(inv => {
                    ...
                    return <option key={inv.id} value={inv.id}>{inv.invoiceNumber} — {client?.name} — ₱{inv.totalAmount.toFixed(2)}</option>;
                  })}
                </select>
              </div>
              <div>
      ...
      {submitted && (
        <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <i className="ti ti-circle-check" style={{ color: '#10B981', fontSize: 20 }} />
          <p style={{ margin: 0, fontWeight: 600, color: '#065F46', fontSize: '0.875rem' }}>Payment recorded successfully and is now pending validation.</p>
          <button onClick={() => setSubmitted(false)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: '#065F46', fontSize: 18 }}>×</button>
        </div>
      )}
      ...
      <TableContainer>
        <DataTable data={enriched} columns={selectedClientId ? columns : columns.filter(c => !['invoiceNumber', 'paymentMethod', 'referenceNumber'].includes(c.key as string))} actions={selectedClientId ? actions : undefined} rowKey="id"
          ...
          searchFields={['invoiceNumber', 'clientName', 'referenceNumber', 'status'] as any}
          filters={[{
      ...
      {viewPayment && createPortal(
        ...
        >
          <div 
            ...
            style={{ 
              width: '100%', maxWidth: '600px', background: '#FFFFFF', borderRadius: '12px', 
              boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', fontFamily: 'var(--fb, var(--font-sans, "Inter", sans-serif))', overflow: 'hidden'
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '24px 32px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                <div style={{ width: '40px', height: '40px', background: '#E6F6F4', color: '#00A99D', display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '10px' }}>
                  <i className="ti ti-cash" style={{ fontSize: '22px' }} />
                </div>
                <h2 style={{ fontSize: '1.2rem', fontWeight: 800, color: '#1E293B', margin: 0 }}>
                  {isFinanceManager ? 'Final Approval' : 'Payment Details'}
                </h2>
              </div>
              <button 
                onClick={() => setViewPayment(null)}
            ...
            <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', maxHeight: '65vh' }}>
              ...
              {!isFinanceManager && viewPayment.status === 'Pending Validation' && (
                <>
                  <div>
            ...
            <div style={{ padding: '24px 32px', display: 'flex', justifyContent: 'flex-end', gap: '16px', background: '#F8FAFC' }}>
              <Button title="Cancel" variant="secondary" onClick={() => setViewPayment(null)} />
              {isFinanceManager && viewPayment.status === 'Validated' ? (
                <Button 
      ...
      {/* View Receipt Modal */}
      {viewReceiptPayment && createPortal(
        <div 
          className="modal-overlay" 
        ...
        >
          <div 
            ...
            style={{ 
              width: '100%', maxWidth: '600px', background: '#FFFFFF', borderRadius: '12px', 
              boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1), 0 8px 10px -6px rgba(0, 0, 0, 0.1)', fontFamily: 'var(--fb, var(--font-sans, "Inter", sans-serif))', overflow: 'hidden'
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '24px 32px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
              ...
              <button onClick={() => setViewReceiptPayment(null)} style={{ width: '36px', height: '36px', background: '#F1F5F9', border: 'none', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#64748B' }}>
                <i className="ti ti-x" style={{ fontSize: 18 }} />
              </button>
            </div>
            ...
            <div style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px', overflowY: 'auto', maxHeight: '65vh' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input type="text" value={`OR-2026-${viewReceiptPayment.invoiceNumber?.slice(-4) || '0000'}`} disabled style={{ padding: '12px 16px', border: '1px solid #FCD34D', borderRadius: '8px', fontSize: '14px', color: '#92400E', background: '#FFFBEB', outline: 'none', fontWeight: 700 }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>DATE ISSUED</label>
                  <input type="text" value={new Date(viewReceiptPayment.recordedAt || Date.now()).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none' }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>CLIENT NAME</label>
                  <input type="text" value={viewReceiptPayment.clientName} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input type="text" value={viewReceiptPayment.invoiceNumber} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none' }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>PAYMENT METHOD</label>
                  <input type="text" value={viewReceiptPayment.paymentMethod} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>REFERENCE NUMBER</label>
                  <input type="text" value={viewReceiptPayment.referenceNumber} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none', fontFamily: 'monospace' }} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <label style={{ fontSize: '11px', fontWeight: 800, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em' }}>AMOUNT RECEIVED</label>
                  <input type="text" value={`₱${viewReceiptPayment.amount.toLocaleString('en-PH', { minimumFractionDigits: 2 })}`} disabled style={{ padding: '12px 16px', border: '1px solid #10B981', borderRadius: '8px', fontSize: '14px', color: '#10B981', fontWeight: 700, background: '#F0FDF4', outline: 'none' }} />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <input type="text" value={viewReceiptPayment.id} disabled style={{ padding: '12px 16px', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '14px', color: '#0F172A', background: '#F8FAFC', outline: 'none', fontWeight: 600 }} />
                </div>
              </div>
            </div>
            <hr style={{ border: 0, borderTop: '1px solid #E2E8F0', margin: 0 }} />
            <div style={{ padding: '24px 32px', display: 'flex', justifyContent: 'flex-end', gap: '16px', background: '#F8FAFC' }}>
              <Button title="Close" variant="secondary" onClick={() => setViewReceiptPayment(null)} />
              <Button 
                title="Print / Download PDF" 
                variant="primary" 
                icon="ti-file-download"
                onClick={() => toast.info(`Downloading PDF Official Receipt for ${viewReceiptPayment.invoiceNumber}...`, 'Download Started')}
              />
            </div>
          </div>
        </div>,
